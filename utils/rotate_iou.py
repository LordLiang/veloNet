...
@cuda.jit(
    ...
    device=True,
    inline=True)
def inter2line_v2(pts1, pts2, i, j, temp_pts):
...
@cuda.jit(
    ...
    device=True,
    inline=True)
def inter2line(pts1, pts2, i, j, temp_pts):
    a = cuda.local.array((2, ), dtype=numba.float32)
    b = cuda.local.array((2, ), dtype=numba.float32)
    c = cuda.local.array((2, ), dtype=numba.float32)
    d = cuda.local.array((2, ), dtype=numba.float32)
...
@cuda.jit('(float32[:], float32[:], float32[:])', device=True, inline=True)
def inter_pts(pts1, pts2, int_pts):
    num_of_inter = 0
    for i in range(4):
    ...
    for i in range(4):
        for j in range(4):
            has_pts = inter2line(pts1, pts2, i, j, temp_pts)
            if has_pts:
...
def convert_region(corners, rbbox):
    # generate clockwise corners and rotate it anticlockwise
    angle = rbbox[4] # 3d:x,y,w,l,r-> 2d:x,y,h,w,r
...
@cuda.jit('(float32[:], float32[:])', device=True, inline=True)
def inter(region1, region2):
    pts1 = cuda.local.array((8, ), dtype=numba.float32)
    pts2 = cuda.local.array((8, ), dtype=numba.float32)
    int_pts = cuda.local.array((16, ), dtype=numba.float32)
...
@cuda.jit('(float32[:], float32[:])', device=True, inline=True)
def devRotateIoU(region1, region2):
    area1 = region1[2] * region1[3]
    area2 = region2[2] * region2[3]
    area_inter = inter(region1, region2)
...
def box2d_rotate_iou(boxes2d, gt_boxes2d, device_id=0):
    ...
    with stream.auto_synchronize():
        ...
        rotate_iou_kernel[blockspergrid, threadsPerBlock, stream](
            N1, N2, boxes_dev, query_boxes_dev, iou_dev)
        iou_dev.copy_to_host(iou.reshape([-1]), stream=stream)

    return iou.astype(boxes2d.dtype)

def box2d_rotate_nms(boxes2d, boxes2d_scores, max_output_size, iou_threshold):
    ...
    scores = np.array(boxes2d_scores)
    order = scores.argsort()[::-1]

    suppressed = np.zeros((num,), dtype=np.bool)

    sorted_boxes2d = boxes2d[order, :]

    iou = box2d_rotate_iou(sorted_boxes2d, sorted_boxes2d)

    for i in range(num):
        if suppressed[i] == True:# drop
            continue
        keep.append(i)
